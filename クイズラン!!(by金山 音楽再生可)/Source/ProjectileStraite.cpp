...
// デストラクタ
ProjectileStraite::~ProjectileStraite() { delete model; }
...
void ProjectileStraite::Update(float elapsedTime)
{
	...
	if (lifeTimer <= 0.0f) Destroy();// 自分を削除
...
// 描画処理
void ProjectileStraite::Render(ID3D11DeviceContext* dc, Shader* shader) { shader->Draw(dc, model); }
